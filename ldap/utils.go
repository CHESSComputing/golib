...
func SearchBy(ldapURL, login, password, baseDN, user, method string, attributes []string) (*ldap.SearchResult, error) {
	...
	} else if method == "gidNumber" {
		filter = fmt.Sprintf("(gidNumber=%s)", ldap.EscapeFilter(user))
	} else if method == "sAMAccountName" {
		filter = fmt.Sprintf("(sAMAccountName=%s)", ldap.EscapeFilter(user))
	} else if method == "cn" {
